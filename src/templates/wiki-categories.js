...
class WikiCategories extends React.Component {
  constructor(props) {
    super(props)
...
WikiCategories.propTypes = propTypes
WikiCategories.defaultProps = defaultProps
export default WikiCategories
export const query = graphql`
  query WikiCategoriesQuery ($category: String ) {
    allMarkdownRemark(limit: 2000, sort: {fields: [frontmatter___created], order: DESC}, filter: {fileAbsolutePath: {glob: "**/content/wiki/**/*.md"}, frontmatter: {categories: {in: [$category]}}}) {
      totalCount
      edges {
        node {
          id
